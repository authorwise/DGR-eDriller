...
import formData from '../../../../json/reportNBT5Hole.json'
...
@Component({
  selector: 'app-report-nbt5-hole',
  templateUrl: './report-nbt5-hole.component.html',
  styleUrls: ['./report-nbt5-hole.component.scss'],
})
export class ReportNbt5HoleComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (this.dataEdit) {
      setTimeout(() => {
        ...
        this.validateForm.get('holeFrom')?.setValue(this.dataEdit.holeFrom);
        this.validateForm
          .get('holeSize')
          ?.setValue(this.dataEdit.holeSize);
        this.validateForm
          .get('holeTo')
          ?.setValue(this.dataEdit.holeTo);
  ...
  submitForm() {
    ...
    this.loadingCtrl
      ...
      .then(async loadingEl => {
        ...
        const config = getApiGEDrillerConfigByKey(this.dataEdit ? 'update_reportNBT5Hole' : "reportNBT5Hole") as any;
  ...
  deleteNBT5Hole(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        ...
        html: 'ไม่สามารถลบข้อมูลหลุมเจาะได้',
        icon: 'error',
        heightAuto: false,
      });
      return;

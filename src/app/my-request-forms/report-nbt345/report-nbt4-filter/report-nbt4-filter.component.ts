...
import formData from '../../../../json/reportNBT4Filter.json'
...
@Component({
  selector: 'app-report-nbt4-filter',
  templateUrl: './report-nbt4-filter.component.html',
  styleUrls: ['./report-nbt4-filter.component.scss'],
})
export class ReportNbt4FilterComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.preFetch = [];
    console.log("ngOnInit : ", this.dataEdit);
  }
  ...
  ngAfterViewInit() {
    ...
    if(this.dataEdit){
      setTimeout(() => {
        ...
        this.validateForm.get('filterFrom')?.setValue(this.dataEdit.filterFrom);
        this.validateForm.get('filterSize')?.setValue(this.dataEdit.filterSize);
        this.validateForm.get('filterTo')?.setValue(this.dataEdit.filterTo);
  ...
  submitForm() {
    ...
    this.loadingCtrl
      ...
      .then(async loadingEl => {
        ...
        const config = getApiGEDrillerConfigByKey(this.dataEdit ? "update_reportNBT4Filter" : "reportNBT4Filter") as any;
        ...
        .subscribe(resData => {
          ...
          console.log('reportNBT4Filter resData', resData);
  ...
  deleteNBT4Pipe(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        ...
        html: 'ไม่สามารถลบข้อมูลท่อกรุได้',
        icon: 'error',
        heightAuto: false,
      });
      return;
      ...
      if (data.isConfirmed == true) {
        ...
        this.loadingCtrl
          ...
          .then(async (loadingEl: any) => {
            ...
            axios.delete(config.url + "/" + this.dataEdit.id, axiosConfig).subscribe(
              (resData) => {
                this.closeLoading(loadingEl);
                let usageSum = 0;
                if (
              ...
              (error) => {
                ...
                Swal.fire({
                  title: 'ข้อผิดพลาด...',
                  html: 'ไม่สามารถดลบข้อมูลท่อกรุได้',
                  icon: 'error',
                  heightAuto: false,
                });
              }
  ...
  deleteNBT4Filter(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        title: 'ไม่สามารถลบข้อมูล...',
        html: 'ไม่สามารถลบข้อมูลขนาดท่อกรองหรือท่อเซาะร่องได้',
        icon: 'error',
        heightAuto: false,
      });
      return;
    ...
    Swal.fire({
      title: 'ลบข้อมูล',
      html: 'ลบข้อมูลขนาดท่อกรองหรือท่อเซาะร่อง',
      icon: 'warning',
      confirmButtonText: 'Delete',
      showCancelButton: true,
      heightAuto: false,
      ...
      if (data.isConfirmed == true) {
        ...
        this.loadingCtrl
          .create({
            keyboardClose: true,
            message: 'กำลังลบข้อมูลขนาดท่อกรองหรือท่อเซาะร่อง...',
          })
          .then(async (loadingEl: any) => {
            loadingEl.present();
            const config = getApiGEDrillerConfigByKey(
              'delete_reportNBT4Filter'
            ) as any;
            const userSult = await this.masterCacheService.getUserInfoByKeys(
              'sult'
            );

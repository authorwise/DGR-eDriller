...
import formData from '../../../../json/reportNBT5Gouge.json';
...
@Component({
  selector: 'app-report-nbt5-gouge',
  templateUrl: './report-nbt5-gouge.component.html',
  styleUrls: ['./report-nbt5-gouge.component.scss'],
})
export class ReportNbt5GougeComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (this.dataEdit) {
      setTimeout(() => {
        ...
        this.validateForm.get('gougeDiameter')?.setValue(this.dataEdit.gougeDiameter);
        this.validateForm
          .get('gougeFrom')
          ?.setValue(this.dataEdit.gougeFrom);
        this.validateForm
          .get('gougeSize')
          ?.setValue(this.dataEdit.gougeSize);
          this.validateForm
          .get('gougeTo')
          ?.setValue(this.dataEdit.gougeTo);
  ...
  submitForm() {
    ...
    this.loadingCtrl
      ...
      .then(async loadingEl => {
        ...
        const config = getApiGEDrillerConfigByKey(this.dataEdit ? "update_reportNBT5Gouge":"reportNBT5Gouge") as any;
        ...
        .subscribe(resData => {
          ...
          console.log('reportNBT5Gouge resData', resData);
  ...
  deleteNBT5Gouge(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        title: 'ไม่สามารถลบข้อมูล...',
        html: 'ไม่สามารถลบข้อมูลการใช้ท่อเซาะร่องได้',
        icon: 'error',
        heightAuto: false,
      });
      return;
    ...
    Swal.fire({
      title: 'ลบข้อมูล',
      html: 'ลบข้อมูลการใช้ท่อเซาะร่อง',
      icon: 'warning',

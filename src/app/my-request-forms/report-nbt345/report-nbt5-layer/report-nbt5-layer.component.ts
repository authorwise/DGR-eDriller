...
import formData from '../../../../json/reportNBT5Layer.json'
...
@Component({
  selector: 'app-report-nbt5-layer',
  templateUrl: './report-nbt5-layer.component.html',
  styleUrls: ['./report-nbt5-layer.component.scss'],
})
export class ReportNbt5LayerComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (this.dataEdit) {
      setTimeout(() => {
        ...
        this.validateForm.get('depthFrom')?.setValue(this.dataEdit.depthFrom);
        this.validateForm
          .get('depthTo')
          ?.setValue(this.dataEdit.depthTo);
        this.validateForm
          .get('layerType')
          ?.setValue(this.dataEdit.layerType);
      });
    } 
  }
  ...
  submitForm() {
    ...
    this.loadingCtrl
      ...
      .then(async loadingEl => {
        ...
        const config = getApiGEDrillerConfigByKey(this.dataEdit ? 'update_reportNBT5Layer': "reportNBT5Layer") as any;
        ...
        .subscribe(resData => {
          ...
          console.log('reportNBT5Hole resData', resData);
  ...
  deleteNBT5Layer(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        title: 'ไม่สามารถลบข้อมูล...',
        html: 'ไม่สามารถลบข้อมูลรายการชั้นดินหรือชั้นหินที่เจาะผ่านได้',
        icon: 'error',
        heightAuto: false,
      });
      return;
    ...
    Swal.fire({
      title: 'ลบข้อมูล',
      html: 'ลบข้อมูลรายการชั้นดินหรือชั้นหินที่เจาะผ่าน',
      icon: 'warning',
      ...
      if (data.isConfirmed == true) {
        ...
        this.loadingCtrl
          .create({
            keyboardClose: true,
            message:
              'กำลังลบข้อมูลรายการชั้นดินหรือชั้นหินที่เจาะผ่าน...',
          })
          .then(async (loadingEl: any) => {
            loadingEl.present();
            const config = getApiGEDrillerConfigByKey(
              'delete_reportNBT5Layer'
            ) as any;

...
import formData from '../../../../json/reportNBT5Pipe.json';
...
@Component({
  selector: 'app-report-nbt5-pipe',
  templateUrl: './report-nbt5-pipe.component.html',
  styleUrls: ['./report-nbt5-pipe.component.scss'],
})
export class ReportNbt5PipeComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (this.dataEdit) {
      setTimeout(() => {
        ...
        this.validateForm.get('pipeDiameter')?.setValue(this.dataEdit.pipeDiameter);
        this.validateForm
          .get('pipeFrom')
          ?.setValue(this.dataEdit.pipeFrom);
        this.validateForm
          .get('pipeTo')
          ?.setValue(this.dataEdit.pipeTo);
  ...
  submitForm() {
    ...
    this.loadingCtrl
      ...
      .then(async loadingEl => {
        ...
        const config = getApiGEDrillerConfigByKey(this.dataEdit ? "update_reportNBT5Pipe" : "reportNBT5Pipe") as any;
        ...
        .subscribe(resData => {
          ...
          console.log('reportNBT5Pipe resData', resData);
  ...
  deleteNBT5Pipe(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        ...
        html: 'ไม่สามารถลบข้อมูลการใช้ท่อกรุได้',
        icon: 'error',
        heightAuto: false,
      });
      return;

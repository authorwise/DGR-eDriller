...
import formData from '../../../../json/reportNBT4Pump.json';
import {
  IFormElement,
  IFormPreFetch,
  IPagingParam,
...
@Component({
  selector: 'app-report-nbt4-pump',
  templateUrl: './report-nbt4-pump.component.html',
  styleUrls: ['./report-nbt4-pump.component.scss'],
})
export class ReportNbt4PumpComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (this.dataEdit) {
      setTimeout(() => {
        ...
        this.validateForm.get('pumpTime')?.setValue(this.dataEdit.pumpTime);
        this.validateForm
          .get('pumpTimeAfter')
          ?.setValue(this.dataEdit.pumpTimeAfter);
        this.validateForm
          .get('reducedWater')
          ?.setValue(this.dataEdit.reducedWater);
      });
    } else {
    ...
    this.loadingCtrl
      ...
      .then(async (loadingEl) => {
        ...
        const config = getApiGEDrillerConfigByKey(
          this.dataEdit ? 'update_reportNBT4Pump' : 'reportNBT4Pump'
        ) as any;
        ...
        axios.post(config.url, formData, axiosConfig).subscribe(
          (resData) => {
            ...
            console.log('reportNBT4Pump resData', resData);
            if (
  ...
  deleteNBT4Pump(): void {
    if (this.dataEdit.id && this.dataEdit.id === '') {
      Swal.fire({
        ...
        html: 'ไม่สามารถลบข้อมูลรายละเอียดการทดสอบขณะทำการสูบน้ำจากบ่อน้ำบาดาลได้',
        icon: 'error',
        heightAuto: false,
      });
      return;
    ...
    Swal.fire({
      title: 'ลบข้อมูล',
      html: 'ลบข้อมูลรายละเอียดการทดสอบขณะทำการสูบน้ำจากบ่อน้ำบาดาล',
      icon: 'warning',
      ...
      if (data.isConfirmed == true) {
        ...
        this.loadingCtrl
          .create({
            keyboardClose: true,
            message:
              'กำลังลบข้อมูลรายละเอียดการทดสอบขณะทำการสูบน้ำจากบ่อน้ำบาดาล...',
          })
          .then(async (loadingEl: any) => {
            loadingEl.present();
            const config = getApiGEDrillerConfigByKey(
              'delete_reportNBT4Pump'
            ) as any;
            const userSult = await this.masterCacheService.getUserInfoByKeys(
              'sult'
            );
